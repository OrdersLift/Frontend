...
const metrics = [
  ...
  { icon: DollarSign, title: 'Profit margin per item', desc: 'Understand which services or products are most profitable and which drain margin.', color: 'text-primary-600 dark:text-primary-400', bg: 'bg-primary-500/10' },
  { icon: Calendar,   title: 'Peak hour analysis',     desc: 'See exactly when you\'re busiest so you can staff accordingly and run better promotions.', color: 'text-accent-700 dark:text-accent-300', bg: 'bg-accent-500/10' },
  ...
  { icon: Users,      title: 'Customer lifetime value', desc: 'Know which customers are worth the most and focus retention efforts where they matter.', color: 'text-primary-600 dark:text-primary-400', bg: 'bg-primary-500/10' },
  { icon: BarChart3,  title: 'Service performance',    desc: 'Rank your services or menu items by revenue, volume, and growth rate.', color: 'text-accent-700 dark:text-accent-300', bg: 'bg-accent-500/10' },
];
...
const SalesInsightsModal = () => (
  <div className="space-y-8 text-neutral-700 dark:text-slate-300">
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-5 rounded-xl bg-cream-100 dark:bg-white/[0.03] border border-primary-200/70 dark:border-white/[0.07]">
        {mockStats.map((s) => (
          <div key={s.label} className="p-3 rounded-lg bg-cream-200 dark:bg-white/[0.04] border border-primary-200/70 dark:border-white/[0.06]">
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {metrics.map((m) => (
          <div key={m.title} className="flex items-start gap-4 p-4 rounded-xl bg-cream-100 dark:bg-white/[0.03] border border-primary-200/70 dark:border-white/[0.06]">
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={r.period} className="p-4 rounded-xl bg-cream-100 dark:bg-white/[0.03] border border-primary-200/70 dark:border-white/[0.06] text-center">
    ...
    {/* Impact */}
    <div className="grid grid-cols-3 gap-4 p-5 rounded-xl bg-gradient-to-r from-glow-500/10 to-accent-500/[0.08] border border-primary-200/70 dark:border-white/[0.06]">
      {[
